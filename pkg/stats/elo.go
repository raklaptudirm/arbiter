...
func PentaSPRT(lls, lds, wldds, wds, wws int, elo0, elo1 float64) (llr float64) {
	...
	r := math.Sqrt(ww*math.Pow(1-mu, 2)+wd*math.Pow(0.75-mu, 2)+wldd*math.Pow(0.5-mu, 2)
			   +ld*math.Pow(0.25-mu, 2)+ll*math.Pow(0-mu, 2))
	if r == 0 {
		return 0
	}
	...
	r0 := math.Sqrt(ww*math.Pow(1-mu0, 2)+wd*math.Pow(0.75-mu0, 2)+wldd*math.Pow(0.5-mu0, 2)
			   +ld*math.Pow(0.25-mu0, 2)+ll*math.Pow(0-mu0, 2))
	r1 := math.Sqrt(ww*math.Pow(1-mu1, 2)+wd*math.Pow(0.75-mu1, 2)+wldd*math.Pow(0.5-mu1, 2)
			   +ld*math.Pow(0.25-mu1, 2)+ll*math.Pow(0-mu1, 2))
...
func PentaElo(lls, lds, wldds, wds, wws int) (muMin float64, mu float64, muMax float64) {
	...
	// standard deviation of the random variable
	sigma := math.Sqrt(ww*math.Pow(1-mu, 2)+wd*math.Pow(0.75-mu, 2)+wldd*math.Pow(0.5-mu, 2)
			   +ld*math.Pow(0.25-mu, 2)+ll*math.Pow(0-mu, 2)) / math.Sqrt(N)
